...
- (id) initWithFirstStageVertexShaderFromString:(NSString *)firstStageVertexShaderString 
    ...
    if (!(self = [super initWithFirstStageVertexShaderFromString: 
            firstStageVertexShaderString ? firstStageVertexShaderString : kGPUImageGaussianBlurHorizontalVertexShaderString
...
- (void) calculateGaussianWeights
{
    ...
    // But useful for debugging to show coefficients in their proper proportion.
    ...
    for (int i = valuesPerSide; i < sizeof(gaussians)/sizeof(GLfloat); i++) {
        ...
        // The standard Gaussian formula uses sigma^2 here, but here we are doing 
        // a linearly decomposed blur along two axes. As it happens, the 
        // linear decomposition of Gaussian(sigma) is Gaussian(sqrt(sigma))
        // in each dimension.
        gaussians[i] = factor * expf(-(n * n)/(2 * _sigma));
        if (i > valuesPerSide) {
            gaussians[2 * valuesPerSide - i] = gaussians[i]; // Symmetric
        }
    }
